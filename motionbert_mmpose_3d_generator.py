...
def parse_args():
    ...
    parser.add_argument('-e', '--evaluate', default='MotionBERT/checkpoint/pose3d/FT_MB_lite_MB_ft_h36m_global_lite/best_epoch.bin', type=str, metavar='FILENAME', help='checkpoint to evaluate (file name)')
...
class MotionBertMMPose3DGenerator:
    ...
    def extract_2d_poses(self, images: Any) -> np.ndarray:
        ...
        for result in results:
            ...
            if result.keypoints is not None and len(result.keypoints.data) > 0:
                ...
                keypoints = result.keypoints.data.cpu().numpy()  # Shape: (num_persons, 17, 3)
                
                for person_keypoints in keypoints:
                    if person_keypoints.shape[0] >= 17:
                        frame_poses.append(person_keypoints[:17])  # Take first 17 keypoints
                    else:
    ...
    def lift_to_3d(self, poses_2d: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            maxlen = int(self.motionbert_model.maxlen) if hasattr(self.motionbert_model, 'maxlen') else poses_tensor.size(0)
            outputs = []
            for start in range(0, poses_tensor.size(0), maxlen):
                end = start + maxlen
                batch_tensor = poses_tensor[start:end].unsqueeze(0)  # Add batch dimension
    ...
    def preprocess_2d_poses(self, poses_2d: np.ndarray) -> np.ndarray:
        ...
        poses_normalized = poses_normalized.reshape(-1, poses_normalized.shape[2], poses_normalized.shape[3])
        ...
        for p in range(P):
            ...
            if len(valid_coords) > 0:
                ...
                if scale > 0:
                    ...
                    poses_normalized[:, p, :, 0] = (poses_normalized[:, p, :, 0] - center_x) / scale * 2
                    poses_normalized[:, p, :, 1] = (poses_normalized[:, p, :, 1] - center_y) / scale * 2
    ...
    def _load_video_frames(self, video_path: str, max_frames: int = 300) -> List[np.ndarray]:
    ...
    def _apply_temporal_smoothing(self, poses_2d: np.ndarray, window_size: int = 5) -> np.ndarray:
