...
def main():
    ...
    parser.add_argument(
        "--validate", type=str, required=True, help="Path to the .pkl file"
    )
    parser.add_argument(
        "--results", type=str, required=True, help="Path to the .pkl file"
    )
    ...
    ground_truth = PlanePoseEstimator().get_pose(
        video.get_angle_stream(CameraAngle.top_view)
    )

    validator = PoseValidation(ground_truth, poses, args.results)
