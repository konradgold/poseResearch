    ...
    def __init__(self, media_path: str):
        ...
        if self.is_directory:
            ...
            for ext in self.image_extensions:
                ...
                self.image_files.extend(glob.glob(os.path.join(media_path, ext.upper())))
...
class GroundTruthLoader:
    ...
    def load_from_json_directory(json_dir: str) -> List[np.ndarray]:
        ...
        json_file_map = {os.path.splitext(os.path.basename(f))[0]: f for f in json_files}
        ...
        if 'people' in json_data and len(json_data['people']) > 0:
            ...
            for person in json_data['people']:
                if 'pose_keypoints_2d' in person:
                    ...
                    keypoints_all.append(np.array(keypoints, dtype=np.float32).reshape(-1, 3))
            if keypoints_all:
                return np.stack(keypoints_all, axis=0)  # shape: (num_people, num_joints, 3)
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Validate YOLO pose predictions against ground truth")
    parser.add_argument('--media', type=str, required=True, help='Path to video file or image directory')
    parser.add_argument('--ground_truth', type=str, required=True, help='Path to ground truth (.pkl file or JSON directory)')
    parser.add_argument('--model', type=str, default='yolo11n-pose.pt', help='Path to YOLO pose model')
    parser.add_argument('--sample_rate', type=int, default=1, help='Frame sampling rate')
    parser.add_argument('--batch_size', type=int, default=8, help='Batch size for YOLO inference')
    parser.add_argument('--output_dir', type=str, default=None, help='Output directory (default: timestamped)')
    ...
    print(f"Aligned sequences: {len(aligned_gt)} ground truth, {len(aligned_yolo)} YOLO predictions")
