...
class SkeletonConfig(ABC):
    ...
    def validate_config(self) -> bool:
        ...
        if len(keypoint_id2name) != num_keypoints:
            raise ValueError(
                f"keypoint_id2name has {len(keypoint_id2name)} entries, expected {num_keypoints}"
            )

        if len(keypoint_name2id) != num_keypoints:
            raise ValueError(
                f"keypoint_name2id has {len(keypoint_name2id)} entries, expected {num_keypoints}"
            )

        if len(keypoint_body_parts) != num_keypoints:
            raise ValueError(
                f"keypoint_body_parts has {len(keypoint_body_parts)} entries, expected {num_keypoints}"
            )
        ...
        for i, name in enumerate(keypoint_names):
            if keypoint_id2name.get(i) != name:
                raise ValueError(
                    f"Inconsistent mapping: keypoint_names[{i}] = '{name}', keypoint_id2name[{i}] = '{keypoint_id2name.get(i)}'"
                )

            if keypoint_name2id.get(name) != i:
                raise ValueError(
                    f"Inconsistent mapping: '{name}' -> {keypoint_name2id.get(name)}, expected {i}"
                )
        ...
        for start_idx, end_idx in skeleton_links:
            if start_idx < 0 or start_idx >= num_keypoints:
                raise ValueError(
                    f"Invalid skeleton link: start_idx {start_idx} out of range [0, {num_keypoints-1}]"
                )
            if end_idx < 0 or end_idx >= num_keypoints:
                raise ValueError(
                    f"Invalid skeleton link: end_idx {end_idx} out of range [0, {num_keypoints-1}]"
                )
