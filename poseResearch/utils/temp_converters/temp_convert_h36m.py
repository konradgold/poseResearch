...
def convert_h36m_to_anatomical(input_file: str, output_file: str = None):
    ...
    try:
        ...
        if h36m_keypoints != expected_keypoints:
            print(
                "Warning: Keypoint names don't match exactly, but proceeding with conversion..."
            )
        ...
        poses_3d_array = np.transpose(
            poses_3d_array, (1, 0, 2, 3)
        )  # Shape: (people, frames, 17, 3)
            ...
            "frame_info": [
                {
                    "frame_id": i,
                    "original_frame_id": (
                        instance_info[i]["frame_id"] if i < len(instance_info) else None
                    ),
                }
