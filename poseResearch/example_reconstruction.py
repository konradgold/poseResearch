...
def test_pose_reconstruction(
    ...
    lifted_data: str = "poseResearch/dataloader",
):
    ...
    for i, quantized_chunk in enumerate(quantized):
        ...
        max_new_tokens = (
            len(quantized[i + 1]) + 1 if i < len(quantized) - 1 else max_new_tokens
        )
        ...
        try:
            print(
                f"Correct tokens: {(torch.tensor(quantized[i+1]) == torch.tensor(out[len(quantized_chunk):len(quantized_chunk) + len(quantized[i+1])])).float().mean()}"
            )
    ...
    if save:
        ...
        output_data = {"poselifting": {"data": [poses_list]}}
