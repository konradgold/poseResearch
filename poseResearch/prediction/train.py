...
train_config = Config.from_yaml(
    "poseResearch/prediction/config/train_gpt2.yaml"
...
def get_batch(split):
    ...
    x = torch.stack(
        [
            torch.from_numpy((data[i : i + train_config.block_size]).astype(np.int64))
            for i in ix
        ]
    )
    y = torch.stack(
        [
            torch.from_numpy(
                (data[i + 1 : i + 1 + train_config.block_size]).astype(np.int64)
            )
            for i in ix
        ]
    )
...
if train_config.init_from == "scratch":
    ...
    model_args["vocab_size"] = (
        meta_vocab_size if meta_vocab_size is not None else train_config.vocab_size
    )
...
scaler = torch.GradScaler(
    device=train_config.device, enabled=(train_config.dtype == "float16")
)
...
raw_model = (
    model.module if ddp else model  # type: ignore
...
while True:
    ...
    if train_config.grad_clip != 0.0:
        ...
        torch.nn.utils.clip_grad_norm_(
            model.parameters(), train_config.grad_clip  # type: ignore
        )
