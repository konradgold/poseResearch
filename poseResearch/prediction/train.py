...
if ddp:
    ...
    seed_offset = 0
    ddp_world_size = 1
tokens_per_iter = train_config.gradient_accumulation_steps * ddp_world_size * train_config.batch_size * train_config.block_size
...
# model init
model_args = dict(n_layer=train_config.n_layer, n_head=train_config.n_head, n_embd=train_config.n_embd, block_size=train_config.block_size,
                  bias=train_config.bias, vocab_size=None, dropout=train_config.dropout) # start with model_args from command line
if train_config.init_from == 'scratch':
    ...
    model_args['vocab_size'] = meta_vocab_size if meta_vocab_size is not None else train_config.vocab_size
    gptconf = GPTConfig(**model_args) # type: ignore
    model = GPT(gptconf)
elif train_config.init_from == 'resume':
    print(f"Resuming training from {train_config.out_dir}")
...
elif train_config.init_from.startswith('gpt2'):
    print(f"Initializing from OpenAI GPT-2 weights: {train_config.init_from}")
    # initialize from OpenAI GPT-2 weights
    override_args = dict(dropout=train_config.dropout)
    model = GPT.from_pretrained(train_config.init_from, override_args)
    # read off the created config params, so we can store them into checkpoint correctly
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias', 'vocab_size']:
        model_args[k] = getattr(model.config, k)
else:
    raise ValueError(f"Unknown init_from value: {train_config.init_from}")
# crop down the model block size if desired, using model surgery
if train_config.block_size < model.config.block_size:
    model.crop_block_size(train_config.block_size)
    model_args['block_size'] = train_config.block_size # so that the checkpoint will have the right value
model.to(train_config.device)

# initialize a GradScaler. If enabled=False scaler is a no-op
scaler = torch.GradScaler(device=train_config.device, enabled=(train_config.dtype == 'float16'))
...
# logging
if train_config.wandb_log and master_process:
    wandb.init(project=train_config.wandb_project, name=train_config.wandb_run_name, config=config)

# training loop
X, Y = get_batch('train') # fetch the very first batch
t0 = time.time()
local_iter_num = 0 # number of iterations in the lifetime of this process
raw_model = model.module if ddp else model # unwrap DDP container if needed # type: ignore
running_mfu = -1.0
while True:
    ...
    for micro_step in range(train_config.gradient_accumulation_steps):
        if ddp:
            ...
            model.require_backward_grad_sync = (micro_step == train_config.gradient_accumulation_steps - 1)  # type: ignore
        with ctx:
            logits, loss = model(X, Y)
    ...
    if iter_num % train_config.log_interval == 0 and master_process:
        ...
        lossf = loss.item() * train_config.gradient_accumulation_steps # type: ignore
        if local_iter_num >= 5: # let the training loop settle a bit
            mfu = raw_model.estimate_mfu(train_config.batch_size * train_config.gradient_accumulation_steps, dt)  # type: ignore
