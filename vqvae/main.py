...
def iterate():
    ...
    for hidden in range(4, 40):
        ...
        model = VQVAE(n_hiddens, args.n_residual_hiddens,
                    args.n_residual_layers, args.n_embeddings, embedding_dim, args.beta).to(device)
        ...
        def train():
            ...
            print('Result for hidden#', hidden, 'Recon Error:',
                np.mean(results["recon_errors"][-args.log_interval:]),
                'Loss', np.mean(results["loss_vals"][-args.log_interval:]),
                'Perplexity:', np.mean(results["perplexities"][-args.log_interval:]),
                'MPJPE:', np.mean(results["mpjpe"]),
                )
